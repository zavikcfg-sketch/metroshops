...
function renderStats(el, data) {
  el.innerHTML = `
    ...
    <motion class="stat-card"><span class="muted">Продажи</span><strong>${data.sales_total} ₽</strong></div>
    <div class="stat-card"><span class="muted">Пользователей</span><strong>${data.users_total}</strong></motion>
  ...
  `.replace(/<\/?motion/g, (m) => (m.includes("</") ? "</motion" : "<motion").replace("motion", "div"));
}
